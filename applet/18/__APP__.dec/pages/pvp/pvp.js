            ...
            var _default = {
                ...
                methods: {
                    next: function next() {
                        ...
                        var url = "../answer/answer?type=" + this.game_type + "&pk_user_id=" + this.pk_user_id + "&enter_type=" + this.enter_type;
                        if (this.type == "special_bot" || this.type == "special_self") {
                    ...
                    pkNext: function pkNext(token) {
                        ...
                        var url = "../answer/answer?type=pk&token=".concat(token, "&pk_user_id=").concat(this.pk_user_id, "&enter_type=").concat(this.enter_type);
                        console.log("-----this.type");
                        console.log(this.type);
                        if (this.type == "special_pk" || this.type == "special_pk_share") {
                            // 专项赛
                            url = "../answer/answer?type=special_pk&token=".concat(token, "&pk_user_id=").concat(this.pk_user_id, "&enter_type=").concat(this.enter_type, "&special_id=").concat(this.special_id);
                        }
                        console.log("-------url");
                        console.log(url);
                        uni.redirectTo({
                ...
                onLoad: function onLoad(options) {
                    ...
                    if (options.type == "pk" || options.type == "special_pk") {
                        ...
                        if (options.type == "special_pk") {
                            request_data.special_id = that.special_id;
                        }
                    ...
                    } else if (options.type == "pk_share" || options.type == "special_pk_share") {
                        this.display_user(user);
                        ...
                        if (that.type == "special_pk_share") {
                            _request_data.special_id = that.special_id;
                        }
                        ...
                        if (options.type == "special_bot" || options.type == "special_self") {
                            _request_data2.type = options.type;
                            _request_data2.special_id = that.special_id;
                        }
                    ...
                    await_timer = setTimeout(function() {
                        if (that.enter_type == "rank_quanzi" && options.type != "pk_share" && options.type != "special_pk_share") {
                            //好友接受挑战进入是从首页进入
                            uni.navigateBack();
                        } else {
                            uni.reLaunch({
